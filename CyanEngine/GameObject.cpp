...
MATERIALSLOADINFO* GameObject::LoadMaterialsInfoFromFile(FILE* pInFile)
{
	char pstrToken[64] = { '\0' };
	UINT nReads = 0;
...
GameObject* GameObject::LoadGeometryFromFile(const char* pstrFileName)
{
	FILE* pInFile = NULL;
	...
	for (; ; )
	{
		...
		{
			pGameObject = GameObject::LoadFrameHierarchyFromFile(pInFile);
		}
...
GameObject* GameObject::LoadFrameHierarchyFromFile(FILE* pInFile)
{
	char pstrToken[64] = { '\0' };
	UINT nReads = 0;
	...
	for (; ; )
	{
		...
		{
			MATERIALSLOADINFO* pMaterialsInfo = pGameObject->LoadMaterialsInfoFromFile(pInFile);
